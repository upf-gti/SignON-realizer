...
FacialController.prototype.start = function (morphTargets) {
    ...
    else if (!this._gazePositions["EYES_TARGET"])
        this._gazePositions["EYES_TARGET"] = lookAtEyesNode.getWorldPosition(new THREE.Vector3());

    if (!this.lookAtHead)
        console.error("LookAt Head not found");
    else if (!this._gazePositions["HEAD_TARGET"])
        this._gazePositions["HEAD_TARGET"] = lookAtHeadNode.getWorldPosition(new THREE.Vector3());

    if (!this.lookAtNeck)
        console.error("LookAt Neck not found");
    else if (!this._gazePositions["NECK_TARGET"])
        this._gazePositions["NECK_TARGET"] = lookAtNeckNode.getWorldPosition(new THREE.Vector3());
