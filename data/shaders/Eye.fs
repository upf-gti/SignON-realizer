...
uniform float u_scleraNormalScale;
...
// Same as in <lights_physical_pars_fragment> but using vertex normal for diffuse (wetness only infuences specular)
void RE_Direct_Physical_VNormal( const in IncidentLight directLight, const in GeometricContext geometry, const in PhysicalMaterial material, inout ReflectedLight reflectedLight ) {
    float dotNL = saturate( dot( vNormal, directLight.direction ) );
	vec3 irradiance = dotNL * directLight.color;
    reflectedLight.directSpecular += irradiance * BRDF_GGX( directLight.direction, geometry.viewDir, geometry.normal, material.specularColor, material.specularF90, material.roughness );
	reflectedLight.directDiffuse += irradiance * BRDF_Lambert( material.diffuseColor );
}
#undef RE_Direct
#define RE_Direct RE_Direct_Physical_VNormal

vec3 GetFinalRadiance(PhysicalMaterial material, vec3 normal)
{
...
PhysicalMaterial GetIrisProperties(vec2 uv)
{
...
PhysicalMaterial GetScleraProperties(vec2 uv)
{
    // ------------ Analogous block to <lights_physical_fragment.glsl> ------------
    // Fill material properties
    PhysicalMaterial material;
...
vec2 GetRefractedUVs(vec2 inUV, vec3 normal)
{
    ...
    vec3 refractedDir = refract(normalize(-vViewPosition), normal, refractionFactor);
    ...
    vec2 refractionUVOffset = (refractedRay).xy;
...
void main() {
    ...
    // Iris
    PhysicalMaterial irisProperties = GetIrisProperties(uv);

    // Sclera
    PhysicalMaterial scleraProperties = GetScleraProperties(vUv);
    // ------------ Analogous block to <normal_fragment_maps> ------------
    vec3 mapN = texture2D( u_scleraNormal, uv ).xyz * 2.0 - 1.0;
	mapN.xy *= u_scleraNormalScale;
    vec3 scleraNormal = perturbNormal2Arb(-vViewPosition, normal, mapN, faceDirection );

    // Mix contribution of both iris and sclera
    PhysicalMaterial material;
    material.diffuseColor = mix(irisProperties.diffuseColor, scleraProperties.diffuseColor, mask);
    material.specularColor = mix(irisProperties.specularColor, scleraProperties.specularColor, mask);
    material.roughness = mix(irisProperties.roughness, scleraProperties.roughness, mask);
    material.specularF90 = mix(irisProperties.specularF90, scleraProperties.specularF90, mask);
    normal = mix(normal, scleraNormal, mask);

    vec3 outputRadiance = GetFinalRadiance(material, normal);
